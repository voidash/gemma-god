...
def _municipality_from_text(text: str, registry_path: str | Path | None) -> LocalityHit | None:
    ...
    for name, aliases in KNOWN_MUNICIPALITY_ALIASES.items():
        if _contains_any(text_l, text, aliases):
            domain = "jirimun.gov.np" if name == "jiri" else "khandbarimun.gov.np"
            district = "dolakha" if name == "jiri" else "sankhuwasabha"
...
def resolve_case(
    ...
    *,
    ...
    service = _detect_service(question) or _detect_service(combined)
    action = _detect_action(question) or _detect_action(combined)
