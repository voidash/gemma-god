...
TOPIC_RULES: dict[str, dict[str, tuple[str, ...]]] = {
    ...
    "birth_registration": {
        "keywords": ("birth", "janmadarta", "janma", "जन्म दर्ता", "जन्म"),
...
LOCALITY_QUERY_TERMS = frozenset({
    "jiri", "jirimun", "जिरी", "नगरपालिका", "nagarpalika", "municipality",
})
...
def _retrieval_rank_features(query: str, row: sqlite3.Row, initial_rank: int) -> dict:
    ...
    local_boost = 7.5 if local_match else 0.0
...
def _retrieval_question(payload: RetrieveRequest | QueryRequest) -> str:
    ...
    previous_user = _last_user_question(payload.history)
...
def _prompt_history(payload: RetrieveRequest | QueryRequest) -> list[ChatHistoryTurn]:
    if _last_user_question(payload.history) and RECHECK_PREVIOUS_RE.search(payload.question.strip()):
        return []
...
def _service_fallback_answer(question: str, gov_results: list[dict], lang: str) -> str | None:
    ...
    """
    topic = _detect_retrieval_topic(question)
    if topic != "birth_registration" or not _detect_local_domains(question):
        return None
    ...
    if lang == "devanagari":
        return (
            f"जिरीमा जन्म प्रमाणपत्रका लागि यसलाई जन्म दर्ता/घटना दर्ताको प्रमाणपत्रको प्रक्रियाका रूपमा लिनुहोस्। "
            ...
            f"जिरी नगरपालिकाको स्रोतले घटना दर्ता सेवा वडा कार्यालयबाट हुने स्थानीय सेवा सन्दर्भ देखाउँछ{local_cite}। "
            ...
            f"Retrieved local स्रोतमा जिरीका लागि छुट्टै birth-specific checklist भेटिएन, त्यसैले आफ्नो सम्बन्धित जिरी वडा कार्यालयमा अन्तिम कागजात/शुल्क पुष्टि गर्नुहोस्{local_cite}।"
        )

    if lang == "roman_nepali":
        return (
            f"Jiri ma birth certificate ko lagi birth registration/event-registration certificate process follow garnu parcha. "
            ...
            f"Jiri Municipality source le event-registration service ward office context ma dekhaucha{local_cite}. "
            ...
            f"Retrieved local source ma Jiri ko separate birth-specific checklist chaina, so final documents/fee afno Jiri ward office ma confirm garnu{local_cite}."
        )

    return (
        f"For Jiri, treat this as birth registration / an event-registration certificate. "
        ...
        f"The Jiri Municipality source gives the local municipality/ward-office context for event-registration services{local_cite}. "
        ...
        f"The retrieved local source does not include a separate Jiri-specific birth checklist, so confirm the final fee/documents at your relevant Jiri ward office{local_cite}."
    )
