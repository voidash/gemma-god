...
RUN_LABEL="sft_v6_4_e4b_g6e_qlora_seed42"
CKPT_REPO="voidash/gemma-helpdesk-v6-4-e4b-g6e-qlora-seed42"
...
cleanup() {
  ...
  hf upload --repo-type model --token "$HF_TOKEN" "$CKPT_REPO" /home/ubuntu/train_v6_4_g6e_qlora.log train_v6_4_g6e_qlora.log
  hf upload --repo-type model --token "$HF_TOKEN" "$CKPT_REPO" /home/ubuntu/eval_v6_4_g6e_qlora.log eval_v6_4_g6e_qlora.log
  echo "=== exit rc=$rc $(date -u +%FT%TZ) ===" | tee -a /home/ubuntu/done_v6_4_g6e_qlora.log
  sudo shutdown -h now
  exit "$rc"
}
trap cleanup EXIT
...
hf download --repo-type dataset --local-dir /home/ubuntu/data --token "$HF_TOKEN" \
  voidash/gemma-helpdesk-data sft_v6_4_train.jsonl sft_v6_4_val.jsonl gov_helpdesk_gold_v1.jsonl
...

{
  ...
  python scripts/train_sft_v1.py \
    --train data/sft_v6_4_train.jsonl \
    --val data/sft_v6_4_val.jsonl \
    --model-id "$BASE_MODEL" \
...
} 2>&1 | tee -a /home/ubuntu/train_v6_4_g6e_qlora.log
...
} 2>&1 | tee -a /home/ubuntu/eval_v6_4_g6e_qlora.log

echo "=== all done $(date -u +%FT%TZ) ===" | tee -a /home/ubuntu/done_v6_4_g6e_qlora.log
