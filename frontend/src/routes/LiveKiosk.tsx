...
function speakableSegment(text: string) {
  ...
  const spoken = selected.join(" ").trim();
...
export function LiveKiosk() {
  ...
  useEffect(() => {
    void checkVoiceProviders().catch(() => undefined);
  }, []);
  ...
  async function measureBackend() {
    ...
    await checkVoiceProviders();
  }

  async function checkVoiceProviders() {
    const providers = await getVoiceProviders();
    const asr = providers.asr_provider || "ASR unknown";
    const tts = providers.tts_enabled ? providers.tts_provider : "TTS off";
    setVoiceStatus(`${asr} · ${tts}`);
    return providers;
  }
  ...
  async function processUtterance(blob: Blob, mime: string) {
    ...
    try {
      ...
      await runQuestion(transcript, asr);
      if (liveEnabledRef.current) setPhaseSafe("listening");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      setTurns((prev) => [...prev, { id: newId(), user: lastTranscript || "(audio)", assistant: "", streaming: false, error: message }]);
      setPhaseSafe("error");
      toast.error(message);
    } finally {
      processingRef.current = false;
  ...
  async function runQuestion(question: string, asr?: VoiceTranscribeResponse) {
  ...
  async function submitTypedQuestion() {
    ...
    try {
      ...
      setPhaseSafe(liveEnabledRef.current ? "listening" : "idle");
    } catch (error) {
      const message = error instanceof Error ? error.message : String(error);
      setTurns((prev) => [...prev, { id: newId(), user: question, assistant: "", streaming: false, error: message }]);
      setPhaseSafe("error");
      toast.error(message);
    } finally {
      processingRef.current = false;
      ...
      <main className="mx-auto grid max-w-6xl gap-4 px-4 py-4 lg:grid-cols-[320px_1fr]">
        <aside className="space-y-4">
          ...
          <section className="rounded-lg border border-slate-300 bg-white p-4 shadow-sm">
            ...
            {voiceStatus && <p className="mt-3 text-xs font-semibold text-emerald-700">{voiceStatus}</p>}
          ...
          >
            <div className="grid gap-2 sm:grid-cols-[1fr_auto]">
              <input
                ...
                placeholder="Type a kiosk question if voice is noisy"
                className="min-h-11 rounded-md border border-slate-300 bg-white px-3 text-base font-semibold outline-none focus:border-emerald-700"
              />
              <button
                type="submit"
                disabled={!typedQuestion.trim() || phase === "thinking" || phase === "transcribing" || phase === "speaking"}
                className="inline-flex min-h-11 items-center justify-center gap-2 rounded-md bg-emerald-700 px-4 text-sm font-extrabold text-white hover:bg-emerald-800 disabled:cursor-not-allowed disabled:bg-slate-300"
              >
